...
def run_split_degree_bucket(degree_split, num_split, step, parameters):
	...
	for step in range(num_split):
		...
		time_list.append([time01-time00, time02-time01, time03-time02])

	return loss_sum, model, time_list
...
def run(args, device, data):
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			for step_out, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				if (degree_split.dim() == 0) and (num_split >= 1):
					split_sum_loss, model, time_sp_list = run_split_degree_bucket(degree_split, num_split, step, parameters)
				...
				if epoch >= args.log_indent:
					...
					epoch_time_list.append(time_9-time0)
		...
		print('avg optimizing time: ', mean(epoch_time_list))
...
def main():
	...
	argparser.add_argument('--num-split-degree', type=float, default=4) # $$$$$$$$$$$$$$$
