...
def invoke_udf_reduce(graph, func, msgdata, *, orig_nid=None):
    ...
    get_memory("----------------------------------------start invoke_udf_reduce() ")
    ...
    get_memory("----------------------------------------before msgdata = Frame(msgdata) ")
    
    msgdata = Frame(msgdata)
    print()
    get_memory("----------------------------------------after msgdata = Frame(msgdata) ")
    
    # degree bucketing
    print('')#
    
    get_memory("----------------------------------------before _bucketing() ")
    
    unique_degs, bucketor = _bucketing(degs)
    get_memory("----------------------------------------after _bucketing() ")
    ...
    for deg, node_bkt, orig_nid_bkt in zip(unique_degs, bucketor(nodes), bucketor(orig_nid)):
        ...
        get_memory("----------------------------------------start msgdata_bkt generation ")
        
        ndata_bkt = dstdata.subframe(node_bkt)
        
        get_memory("----------------------------------------after ndata_bkt = dstdata.subframe(node_bkt) ")
        ...
        get_memory("----------------------------------------after msgdata_bkt generation ")
        ...
        get_memory("----------------------------------------after msgdata_bkt for loop ")
        get_memory("----------------------------------------before NodeBatch() ")
        # invoke udf
        nbatch = NodeBatch(graph, orig_nid_bkt, ntype, ndata_bkt, msgs=maildata)
        
        
        get_memory("----------------------------------------after NodeBatch() ")
        
        bkt_rsts.append(func(nbatch))
        
        get_memory("----------------------------------------after reduce func(NodeBatch) ")
        
        
    
    get_memory("----------------------------------------before Frame()  ")
...
def get_memory(message, force=True):
	logger = ''
	logger += message
	nvmlInit()
