...
def invoke_udf_reduce(graph, func, msgdata, degree, num_split, step, *, orig_nid=None, ):
    ...
    see_memory_usage("----------------------------------------start invoke_udf_reduce() ")
    ...
    see_memory_usage("----------------------------------------before msgdata = Frame(msgdata) ")
    msgdata = Frame(msgdata)
    see_memory_usage("----------------------------------------after msgdata = Frame(msgdata) ")
    # degree bucketing
    see_memory_usage("----------------------------------------before _bucketing() ")
    ...
    see_memory_usage("----------------------------------------after _bucketing() ")
    bkt_rsts = []
    bkt_nodes = []
    for deg, node_bkt, orig_nid_bkt in zip(unique_degs, bucketor(nodes), bucketor(orig_nid)):
        ...
        see_memory_usage("----------------------------------------before bkt_nodes.append(node_bkt) ")
        bkt_nodes.append(node_bkt)
        see_memory_usage("----------------------------------------start msgdata_bkt generation ")
        ndata_bkt = dstdata.subframe(node_bkt)
        see_memory_usage("----------------------------------------after ndata_bkt = dstdata.subframe(node_bkt) ")
        ...
        see_memory_usage("----------------------------------------after msgdata_bkt generation ")
        ...
        see_memory_usage("----------------------------------------before NodeBatch() ")
        # invoke udf
        nbatch = NodeBatch(graph, orig_nid_bkt, ntype, ndata_bkt, msgs=maildata)
        see_memory_usage("----------------------------------------after NodeBatch() ")
        
        bkt_rsts.append(func(nbatch))
        see_memory_usage("----------------------------------------after reduce func(NodeBatch) ")
        
    see_memory_usage("----------------------------------------Before Frame()  ")
    ...
    see_memory_usage("----------------------------------------after merge nodes ")
    return retf
...
def message_passing(g, mfunc, rfunc, afunc, degree, num_split, step):
    ...
    if (is_builtin(mfunc) and is_builtin(rfunc) and
        ...
        # message phase
        if is_builtin(mfunc):
            see_memory_usage("----------------------------------------before msgdata gen ")
            msgdata = invoke_gsddmm(g, mfunc)
            see_memory_usage("----------------------------------------after msgdata gen ")
        else:
...
# I added part for GPU mem usage profiling
import torch
