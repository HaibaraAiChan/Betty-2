...
def invoke_udf_reduce(graph, func, msgdata, *, orig_nid=None):
    ...
    for deg, node_bkt, orig_nid_bkt in zip(unique_degs, bucketor(nodes), bucketor(orig_nid)):
        ...
        see_memory_usage("----------------------------------------after msgdata_bkt for loop ")
...
def _bucketing(val):
    ...
    # print('len of sorted_val', len(sorted_val))
    # print() 
    # tp2 = time.time()#
    ...
    # tp4 = time.time()#
    # print('sorted_val, idx = F.sort_1d(val) ', tp2-tp1)
    # print('tunique_val = F.asnumpy(F.unique(sorted_val)) ', tp3-tp2)
    # print('eqidx = F.nonzero_1d(F.equal(sorted_val, v)) ', tp4-tp3)
...
def message_passing(g, mfunc, rfunc, afunc):
    ...
    if (is_builtin(mfunc) and is_builtin(rfunc) and
        ...
        if is_builtin(rfunc):
            ...
            time2 = time.time()
            print('-----------------------invoke_udf_reduce spend: ', time2-time1)
    # apply phase
