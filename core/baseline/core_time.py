...
def invoke_udf_reduce(graph, func, msgdata, *, orig_nid=None):
    ...
    print('get ready time : ', time1-time0)#
    unique_degs, bucketor = _bucketing(degs)
    time2 = time.time()
    print('_buckting time : ', time2-time1)#
    bkt_rsts = []
    bkt_nodes = []
    for deg, node_bkt, orig_nid_bkt in zip(unique_degs, bucketor(nodes), bucketor(orig_nid)):
        ...
        print('ndata_bkt = dstdata.subframe(node_bkt): ', tt2-tt1)
        print('msgdata_bkt generation: ', tt3-tt2)
        print('msgdata_bkt for loop: ', tt4-tt3)
        print('NodeBatch generation: ', tt5-tt4)
        print('func(nbatch): ', tt6-tt5)
    time3 = time.time()
    ...
    time5 = time.time()
    print('--------------merge nodes generation time : ', time5-time4)
    return retf
