		...
		for epoch in range(args.num_epochs):
			...
			for step, (input_nodes, seeds, blocks) in enumerate(block_dataloader):
				...
				for bi in blocks:
					...
					global_eids = bi.edata['_ID']
					global_srcnid = bi.srcdata['_ID']
					global_dstnid = bi.dstdata['_ID']
					local_s = bi.edges(form='all')[0]
					local_e = bi.edges(form='all')[1]
					num_src_nodes = len(bi.srcdata['_ID'])
					num_dst_nodes = len(bi.dstdata['_ID'])
					cur_block = dgl.create_block((local_s,local_e), num_src_nodes=num_src_nodes, num_dst_nodes=num_dst_nodes)
					cur_block.edata['_ID'] = global_eids
					cur_block.srcdata['_ID'] = global_srcnid
					cur_block.dstdata['_ID'] = global_dstnid
