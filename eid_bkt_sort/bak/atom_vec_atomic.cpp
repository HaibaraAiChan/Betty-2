  ...
  if (pbc_flag != 0) {
    ...
    if (space==Host) {
      AtomVecAtomicKokkos_PackBorder<LMPHostType,1> f(
        ...
        printf("host   n = %d, (x,y,z) = %f,%f,%f \n ", n, dx, dy, dz);
      Kokkos::parallel_for(n,f);
      printf("Kokkos::parallel_for(n,f); f._dx, f._dy, f._dz  %d %d %d \n ", f._dx, f._dy, f._dz);
    } else {
      AtomVecAtomicKokkos_PackBorder<LMPDeviceType,1> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        iswap,d_x,d_tag,d_type,d_mask,dx,dy,dz);
        printf("device  n = %d, (x,y,z) = %f,%f,%f \n ", n, dx, dy, dz);
      Kokkos::parallel_for(n,f);
      printf("Kokkos::parallel_for(n,f); f._dx, f._dy, f._dz  %d %d %d \n ", f._dx, f._dy, f._dz);
    }
    ...
    if (space==Host) {
      ...
      AtomVecAtomicKokkos_PackBorder<LMPDeviceType,0> f(
        ...
        printf( "device  n = %d, (x,y,z) = %f,%f,%f \n", n, dx, dy, dz);
      Kokkos::parallel_for(n,f);
      // printf("Kokkos::parallel_for(n,f);&f._buff  %p \n ", f._buff);
      // printf("f._buff %p \n",f->_buff[0] );
      // len = sizeof(f->_buff) / sizeof(f->_buff[0]);
      // Print_array(f->_buff, len );
      // printf("Kokkos::parallel_for(n,f); f._list    \n ", );
      // Print_array(f._list, sizeof(f._list) / sizeof(f._list[0]));
      // printf("Kokkos::parallel_for(n,f); f._x     %d \n ",  );
      // Print_array(f._x, sizeof(f._x) / sizeof(f._x[0]));
      // printf("Kokkos::parallel_for(n,f); f._tag   \n ", );
      // Print_array( f._tag, sizeof( f._tag) / sizeof( f._tag[0]));
      // printf("Kokkos::parallel_for(n,f); f._type   \n ",);
      // Print_array( f._type, sizeof(  f._type) / sizeof(  f._type[0]));
      // printf("Kokkos::parallel_for(n,f); f._mask  %d \n ",  f._mask);
      printf("Kokkos::parallel_for(n,f); f._dx, f._dy, f._dz  %d %d %d \n ", f._dx, f._dy, f._dz);
...
void AtomVecAtomicKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  if (space==Host) {
    ...
    printf( "host   h_x = %s, h_tag= %d, h_type= %d, h_mask= %d, first = %d \n ", h_x,h_tag,h_type,h_mask,first);
    Kokkos::parallel_for(n,f);
    printf("after Kokkos::parallel_for(n,f);  h_x = %s, h_tag= %d, h_type= %d, h_mask= %d, first = %d \n ", f._x,f._tag,f._type,f._mask,f._first);
  
  } else {
    struct AtomVecAtomicKokkos_UnpackBorder<LMPDeviceType> f(buf.view<LMPDeviceType>(),d_x,d_tag,d_type,d_mask,first);
    printf( "device   h_x = %s, h_tag= %d, h_type= %d, h_mask= %d, first = %d \n ", h_x,h_tag,h_type,h_mask,first);
    Kokkos::parallel_for(n,f);
    printf("after Kokkos::parallel_for(n,f);  h_x = %s, h_tag= %d, h_type= %d, h_mask= %d, first = %d \n ", f._x,f._tag,f._type,f._mask,f._first);
  
  }
  // printf("Kokkos::parallel_for(n,f);, %p \n ", &f);
}
...
double AtomVecAtomicKokkos::memory_usage()
{
  ...
  printf("AtomVecAtomicKokkos::memory_usage %d \n ", bytes);
  return bytes;
}
...
{
  // printf("AtomVecAtomicKokkos::sync %d \n ", 100);
  
  if (space == Device) {
    printf("AtomVecAtomicKokkos::sync space %s \n ", "device");
  ...
  // printf(" atomKK->k_type.d_view %d \n ", atomKK->k_type.d_view);
  // printf(" atomKK->k_type.h_view %d \n ", atomKK->k_type.h_view);
}
...
{
  printf("AtomVecAtomicKokkos::sync_overlapping_device %d \n ", 100);
  if (space == Device) {
...
{
  // printf("AtomVecAtomicKokkos::modified %d \n ", 100);
  // int ix,iy,iz;
  if (space == Device) {
    printf("AtomVecAtomicKokkos::modified space %s \n ", "device");
    ...
    // DAT::t_x_array_const h_x = atomKK->k_x.view<LMPDeviceType>();
    // printf(" DAT::t_x_array_const h_x = atomKK->k_x.view<LMPDeviceType>(); %d \n ", &h_x);
    // ix = static_cast<int>(h_x(0, 0));
    // iy = static_cast<int>(h_x(0, 1));
    // iz = static_cast<int>(h_x(0, 2));
    // printf(" atomKK->k_type.h_view(ix,iy,iz)= (%d, %d, %d) \n ", ix,iy,iz);
    ...
    // if (mask & X_MASK) printf("atomKK->k_x  modify \n " );
    // if (mask & V_MASK) printf("atomKK->k_v.modify<LMPHostType>()\n " );
    // if (mask & F_MASK) printf("atomKK->k_f.modify<LMPHostType>()\n " );
    // if (mask & TAG_MASK) printf("atomKK->k_tag.modify<LMPHostType>()\n " );
    // if (mask & TYPE_MASK) printf("atomKK->k_type.modify<LMPHostType>()\n " );
    // if (mask & MASK_MASK) printf("atomKK->k_mask.modify<LMPHostType>()\n " );
    // if (mask & IMAGE_MASK) printf("atomKK->k_image.modify<LMPHostType>()\n " );

    // HAT::t_x_array_const h_x = atomKK->k_x.view<LMPHostType>();
    // ofstream outFile("atomKK->k_x.bin", ios::out|ios::binary);
    // outFile.write((const char *)& atomKK->k_x, sizeof((atomKK->k_x)));
    // outFile.close();

    // printf(" HAT::t_x_array_const h_x = atomKK->k_x.view<LMPHostType>(); %d \n ", *h_x);
    // printf("h_x(0, 0)  %f \n ",atomKK->k_x.view<LMPHostType>()(0, 0) );
    // ix =  (h_x(0, 0));
    // iy =  (h_x(0, 1));
    // iz =  (h_x(0, 2));
    // printf(" atomKK->k_type.h_view(ix,iy,iz)= (%f, %f, %f) \n ", ix,iy,iz);
  }
  // printf(" atomKK->k_type.d_view %d \n ", atomKK->k_type.d_view);
  // printf(" atomKK->k_type.h_view %d \n ", atomKK->k_type.h_view);
